                             ...
                             endpoint_t * endP)
{
    ...
    case BS_WRITE_SECURITY:
    {
        ...
        // uri.instanceId = endP->cmdList->serverId; // org
        uri.instanceId = 0; // young change

        fprintf(stdout, "Sending WRITE Security");
    ...
    case BS_WRITE_SERVER:
    {
        ...
        fprintf(stdout, "Sending WRITE Server");
        prv_print_uri(stdout, &uri);
        fprintf(stdout, " to \"%s\"", endP->name);
...
static int prv_bootstrap_callback(lwm2m_context_t *lwm2mH, void *sessionH, uint8_t status, lwm2m_uri_t *uriP,
    ...
    case COAP_NO_ERROR:
    {
        ...
        // Nothing found, reject the request.
        if (endInfoP == NULL)return COAP_400_BAD_REQUEST;
