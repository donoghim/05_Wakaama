...
bs_info_t *  bs_get_info(FILE * fd)
{
    ...
    if (infoP == NULL) {errCode=1; return NULL;}
    ...
    do
    {
        ...
        if (readSrvP != NULL)
        {
            if (prv_add_server(infoP, readSrvP) != 0) {errCode=2; goto error;}
        }
    } while (readSrvP != NULL);
    ...
    // check validity
    if (infoP->endpointList == NULL) {errCode=3; goto error;}
    ...
    while (cltInfoP != NULL)
    {
        ...
        while (otherP != NULL)
        {
            ...
            {
                if (otherP->name == NULL) {errCode=4; goto error;}
            }
            else
            {
                if (otherP->name != NULL
        ...
        while (cmdP != NULL)
        {
            ...
            case BS_WRITE_SECURITY:
                ...
                {
                  fprintf(stderr, "err. server id=%d\r\n", cmdP->serverId);
                  errCode=6; goto error;
                }
                else
                {
                  fprintf(stdout, "ok. server id=%d\r\n", cmdP->serverId);
                }
                parentP = cmdP;
                cmdP = cmdP->next;
                break;
            ...
            case BS_WRITE_SERVER:
            {
                ...
                if (serverP == NULL) {errCode=7; goto error;} 
...
error:
    fprintf(stderr, "bs_get_info() ec=%d\r\n", errCode);
    bs_free_info(infoP);
    return NULL;
}
