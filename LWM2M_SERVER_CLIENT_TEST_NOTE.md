...
## 4.1 클라이언트에서 31024 값 변경

클라이언트 콘솔의 `change` 명령으로 `31024` 리소스 값을 변경할 수 있다.
서버에서 해당 리소스를 Observe 중이면 값이 변경될 때 클라이언트가 Notify를
서버로 전송한다.

먼저 서버 콘솔에서 `31024`의 정수 리소스를 Observe한다.

```text
observe 0 /31024/10/1
```

그 다음 클라이언트 콘솔에서 현재 값 `20`을 `42`로 변경한다.

```text
change /31024/10/1 42
```

서버에는 다음과 비슷한 Notify 출력이 표시된다.

```text
Notify from client #0 /31024/10/1
```

서버에서 변경된 값을 다시 읽어 확인할 수도 있다.

```text
read 0 /31024/10/1
```

다른 `31024` 리소스도 같은 방식으로 변경할 수 있다.

```text
change /31024/11/3 -12.5
change /31024/12/5 hello
```

테스트가 끝나면 서버 콘솔에서 Observe를 해제한다.
