...
echo "----- start LWM2M server IPv4 port ${LWM2M_PORT} in commercial registration mode"
echo "----- control socket /tmp/lwm2mserver-control.sock"
...
exec ../../build/server/lwm2mserver -4 -C -l "${LWM2M_PORT}" -p /tmp/lwm2mserver-control.sock -F "${DFOTA_FW_ROOT}" -u "${DFOTA_URI_PREFIX}" -H "${DFOTA_HOST}"